...
`artifact_threshold` (default: 10) as unreliable and replaces them with zero
before capping the remaining counts at the model-supported value of 5. The
threshold is a heuristic for possible mapping errors or unusual fragment
structures; use `artifact_threshold = Inf` to disable it and consider a
sensitivity analysis when high counts are common in your assay.

## Vignettes

The vignettes can be found at our `pkgdown` page:

[https://zhen-miao.github.io/PICsnATAC/](https://zhen-miao.github.io/PICsnATAC/)


## Interactive Notebooks
_Here, we provided two Google Colab notebooks for users to run the codes on cloud (processed example data and link provided). However, as snATAC-seq data are typically large and computer intensive, it is recommended to run it using computing cluster for large datasets_
...
Miao Z, and Kim J. *Uniform quantification of single-nucleus ATAC-seq data with Paired-Insertion Counting (PIC) and a model-based insertion rate estimator.* _Nature Methods_ 21.1 (2024): 32-36. [doi:10.1038/s41592-023-02103-7](https://doi.org/10.1038/s41592-023-02103-7).
